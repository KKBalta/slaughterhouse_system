...
class DispositionUpdateTest(TestCase):
    ...
    def setUp(self):
        ...
        self.animal = Animal.objects.create(
            slaughter_order=self.order,
            animal_type='cattle',
            identification_tag='DISP-TEST-001'
        )
        self.animal.perform_slaughter()
        self.animal.save()
...
class TestInventoryQueries:
    ...
    def test_get_inventory_by_location(
        ...
        Carcass.objects.create(
            animal=animal,
            hot_carcass_weight=Decimal('200'),
            disposition='for_sale',
    ...
    def test_get_inventory_for_animal(
        self, animal_factory, service_package_factory
    ):
