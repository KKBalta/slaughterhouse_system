...
class InventoryModelTest(TestCase):
    ...
    def test_create_carcass(self):
        carcass = Carcass.objects.create(animal=self.animal, hot_carcass_weight=250.75, disposition="for_sale")
    ...
    def test_create_meat_cut(self):
        carcass = Carcass.objects.create(animal=self.animal, hot_carcass_weight=250.75, disposition="returned_to_owner")
        ...
        self.assertEqual(
            str(meat_cut), f"{meat_cut.cut_type} from {carcass.animal.identification_tag} - {meat_cut.weight} kg"
        )

    def test_create_offal(self):
        offal = Offal.objects.create(
            animal=self.animal, offal_type=Offal.BeefOffalTypes.LIVER, weight=5.2, disposition="for_sale"
        )
    ...
    def test_create_by_product(self):
        by_product = ByProduct.objects.create(
            animal=self.animal, byproduct_type=ByProduct.ByProductTypes.SKIN, disposition="disposed"
        )
    ...
    def test_animal_cascade_delete(self):
        Offal.objects.create(
            animal=self.animal, offal_type=Offal.BeefOffalTypes.HEART, weight=2, disposition="for_sale"
        )
        ByProduct.objects.create(
            animal=self.animal, byproduct_type=ByProduct.ByProductTypes.FEET, disposition="disposed"
        )
    ...
    def test_reverse_relationships(self):
        ...
        mc2 = MeatCut.objects.create(
            carcass=carcass, cut_type=MeatCut.BeefCuts.SHANK, weight=2.5, disposition="for_sale"
        )
        offal = Offal.objects.create(
            animal=self.animal, offal_type=Offal.BeefOffalTypes.KIDNEY_FAT, weight=0.5, disposition="disposed"
        )
        by_product = ByProduct.objects.create(
            animal=self.animal, byproduct_type=ByProduct.ByProductTypes.HEAD, weight=20, disposition="disposed"
        )
