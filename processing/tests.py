...
class ProcessingModelTest(TestCase):
    def setUp(self):
        ...
        self.service_package_simple = ServicePackage.objects.create(
            name="Slaughter Only", includes_disassembly=False, includes_delivery=False
        )
        self.order = SlaughterOrder.objects.create(
            client=self.client_profile, order_datetime=timezone.now(), service_package=self.service_package_full
        )

    def test_create_animal(self):
        animal = Animal.objects.create(
            slaughter_order=self.order, animal_type="cattle", identification_tag="CATTLE-001"
        )
    ...
    def test_received_date_editable(self):
        ...
        animal = Animal.objects.create(slaughter_order=self.order, animal_type="cattle", received_date=past_datetime)
    ...
    def test_animal_fsm_transitions(self):
        animal = Animal.objects.create(
            slaughter_order=self.order, animal_type="cattle", identification_tag="CATTLE-002"
        )
        self.assertEqual(animal.status, "received")
        ...
        WeightLog.objects.create(animal=animal, weight=300.0, weight_type="hot_carcass_weight", is_group_weight=False)
    ...
    def test_animal_fsm_conditional_transition_fail(self):
        simple_order = SlaughterOrder.objects.create(
            client=self.client_profile, order_datetime=timezone.now(), service_package=self.service_package_simple
        )
        animal = Animal.objects.create(
            slaughter_order=simple_order, animal_type="sheep", identification_tag="SHEEP-001"
        )
        animal.perform_slaughter()
        animal.prepare_carcass()
    ...
    def test_create_animal_details(self):
        animal = Animal.objects.create(
            slaughter_order=self.order, animal_type="cattle", identification_tag="CATTLE-003"
        )
        cattle_details = CattleDetails.objects.create(
            animal=animal, breed="Angus", sakatat_status=1.0, bowels_status=0.0
        )
    ...
    def test_animal_leather_weight_kg(self):
        animal = Animal.objects.create(slaughter_order=self.order, animal_type="cattle", leather_weight_kg=150.75)
        self.assertEqual(animal.leather_weight_kg, 150.75)

        animal_sheep = Animal.objects.create(slaughter_order=self.order, animal_type="sheep", leather_weight_kg=10.20)
        self.assertEqual(animal_sheep.leather_weight_kg, 10.20)

    def test_create_individual_weight_log(self):
        animal = Animal.objects.create(slaughter_order=self.order, animal_type="goat", identification_tag="GOAT-001")
    ...
    def test_weight_log_constraints(self):
        ...
        with self.assertRaises(Exception):
            WeightLog.objects.create(slaughter_order=self.order, weight=10.0, weight_type="Test", is_group_weight=True)
