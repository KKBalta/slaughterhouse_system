...
class AnimalAdmin(admin.ModelAdmin):
    list_display = (
        ...
        "slaughter_date",
        "get_leather_weight",
        "get_latest_hot_carcass_weight",
        "get_picture_status",
        "get_scale_receipt_picture",
    )
    ...
    def get_latest_hot_carcass_weight(self, obj):
        ...
        hot_carcass_log = (
            obj.individual_weight_logs.filter(weight_type="hot_carcass_weight").order_by("-log_date").first()
        )
    ...
    def get_scale_receipt_picture(self, obj):
        if obj.scale_receipt_picture:
            return format_html(
                '<a href="{}" target="_blank"><img src="{}" style="max-height:40px;max-width:60px;border-radius:4px;"/></a>',
                obj.scale_receipt_picture.url,
                obj.scale_receipt_picture.url,
            )
        return "-"
