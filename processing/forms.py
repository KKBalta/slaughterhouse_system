            ...
            if weight_type_lower in ["hot_carcass_weight"]:
                if self.animal.status not in ["slaughtered", "carcass_ready"]:
                    raise ValidationError(
                        {
                            "weight_type": _(
                                "Hot carcass weight can only be logged for slaughtered animals. Animal %(tag)s is currently %(status)s."
                            )
                            % {"tag": self.animal.identification_tag, "status": self.animal.get_status_display()}
                        }
                    )
                ...
                if self.animal.status not in ["carcass_ready", "disassembled", "packaged", "delivered"]:
                    raise ValidationError(
                        {
                            "weight_type": _(
                                "Cold carcass weight can only be logged for animals with carcass ready or later status. Animal %(tag)s is currently %(status)s."
                            )
                            % {"tag": self.animal.identification_tag, "status": self.animal.get_status_display()}
                        }
                    )
                ...
                if self.animal.status not in ["disassembled", "packaged", "delivered"]:
                    raise ValidationError(
                        {
                            "weight_type": _(
                                "Final weight can only be logged for disassembled animals. Animal %(tag)s is currently %(status)s."
                            )
                            % {"tag": self.animal.identification_tag, "status": self.animal.get_status_display()}
                        }
                    )
                ...
                if self.animal.status == "received":
                    raise ValidationError(
                        {
                            "weight_type": _(
                                "Leather weight should be logged after slaughter. Animal %(tag)s is currently %(status)s."
                            )
                            % {"tag": self.animal.identification_tag, "status": self.animal.get_status_display()}
                        }
                    )
...
class ScaleReceiptUploadForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "scale_receipt_picture": forms.FileInput(
                attrs={
...
class BatchWeightLogForm(forms.Form):
    ...
    order_id = forms.UUIDField(widget=forms.HiddenInput(attrs={"style": "display: none;"}))
            ...
            if order_id and weight_type:
                try:
                    ...
                    if animal_count > available_count:
                        raise ValidationError(
                            _(
                                "Cannot log weight for %(animal_count)s animals. Only %(available_count)s animals are available for %(weight_type)s weighing in this order."
                            )
                            % {
                    ...
                    if total_after_this_batch > available_count:
                        ...
                        raise ValidationError(
                            _(
                                "Cannot log weight for %(animal_count)s animals. Only %(remaining_available)s animals remain available for %(weight_type)s weighing (%(already_weighed)s already weighed out of %(total)s total)."
                            )
                            % {
                                "animal_count": animal_count,
...
class CattleDetailsForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "sakatat_status": forms.Select(
                attrs={
                    "class": "w-full border border-gray-300 rounded-md px-3 py-2 text-sm text-gray-900 bg-white focus:ring-blue-500 focus:border-blue-500"
                }
            ),
            "bowels_status": forms.Select(
                attrs={
                    "class": "w-full border border-gray-300 rounded-md px-3 py-2 text-sm text-gray-900 bg-white focus:ring-blue-500 focus:border-blue-500"
                }
            ),
...
class DisassemblyCutForm(forms.ModelForm):
    ...
    def clean(self):
        cleaned_data = super().clean()
        if self.animal and self.animal.is_boneless_disassembly():
            # Ensure cut_name is set to boneless_meat for boneless disassembly
            cleaned_data["cut_name"] = "boneless_meat"

        # Validate that total disassembly cuts weight doesn't exceed hot carcass weight
        if self.animal and cleaned_data.get("weight_kg"):
            # Get hot carcass weight
            hot_carcass_log = (
                self.animal.individual_weight_logs.filter(weight_type="hot_carcass_weight", is_group_weight=False)
                .order_by("-log_date")
                .first()
            )
            ...
            if hot_carcass_log and hot_carcass_log.weight:
                ...
                new_weight = float(cleaned_data["weight_kg"])

                # Calculate total weight of existing cuts (excluding the one being edited if editing)
                existing_cuts = self.animal.disassembly_cuts.all()
                if self.instance and self.instance.pk:
                    # If editing, exclude the current cut from the total
                    existing_cuts = existing_cuts.exclude(pk=self.instance.pk)
                ...
                if total_weight_after > hot_carcass_weight:
                    ...
                    raise ValidationError(
                        _(
                            "Total disassembly weight (%(total)s kg) cannot exceed hot carcass weight (%(carcass)s kg). "
