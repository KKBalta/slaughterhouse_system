...
def scale_receipt_upload_path(instance, filename):
    ...
    ext = filename.split(".")[-1] if "." in filename else "jpg"
    tag = (
        instance.identification_tag
        if instance.identification_tag
        else f"{instance.animal_type.upper()}-{uuid.uuid4().hex[:10].upper()}"
    )
    clean_tag = "".join(c for c in tag if c.isalnum() or c in ("-", "_")).rstrip()
...
class Animal(BaseModel):
    ...
    slaughter_date = models.DateTimeField(
        null=True, blank=True, help_text=_("Date and time the animal was slaughtered.")
    )
    status = FSMField(
        default="received",
    ...
    picture = models.ImageField(
        upload_to=animal_picture_upload_path, blank=True, null=True, help_text=_("Picture of the animal.")
    )
    ...
    leather_weight_kg = models.DecimalField(
        max_digits=6, decimal_places=2, null=True, blank=True, help_text=_("The weight of the leather in kilograms.")
    )
    ...
    def get_performance(self):
        ...
        hot_carcass_log = (
            self.individual_weight_logs.filter(weight_type="hot_carcass_weight").order_by("-log_date").first()
        )
    ...
    def can_proceed_to_disassembly(self):
        ...
        has_disassembly_service = (
            self.slaughter_order.service_package.includes_disassembly
    ...
    def is_eligible_for_disassembly(self):
        ...
        if self.status == "carcass_ready":
            return self.individual_weight_logs.filter(weight_type="hot_carcass_weight", is_group_weight=False).exists()
        return False
...
class WeightLog(BaseModel):
    ...
    weight = models.DecimalField(
        ...
        decimal_places=2,
        help_text=_(
            "Weight recorded (e.g., live weight, hot carcass weight). For group weights, this is the average per animal."
        ),
    )
    ...
    is_group_weight = models.BooleanField(
        default=False, help_text=_("True if this log entry represents a group weighing.")
    )
    group_quantity = models.IntegerField(
        null=True, blank=True, help_text=_("Number of animals in the group, if this is a group weight.")
    )
    ...
    log_date = models.DateTimeField(auto_now_add=True, help_text=_("Date and time the weight was logged."))
...
class DisassemblyCut(BaseModel):
    ...
    animal = models.ForeignKey(Animal, on_delete=models.CASCADE, related_name="disassembly_cuts")
    ...
    cut_name = models.CharField(max_length=100, help_text=_("Name of the cut (depends on animal type)"))

    weight_kg = models.DecimalField(max_digits=7, decimal_places=2, help_text=_("Weight of this cut in kg"))
    ...
    def save(self, *args, **kwargs):
        ...
        if self.animal.is_boneless_disassembly() and self.cut_name != "boneless_meat":
            raise ValidationError(
                _("This animal uses boneless disassembly. Only 'Boneless Meat (Total)' can be recorded.")
            )
