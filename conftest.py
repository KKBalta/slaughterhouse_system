...
class ClientProfileFactory(factory.django.DjangoModelFactory):
    ...
    user = factory.SubFactory(UserFactory, role=factory.LazyAttribute(
        lambda obj: __import__('users.models', fromlist=['User']).User.Role.CLIENT
    ))
    ...
    def create_package(
        ...
        **kwargs
        ...
        return ServicePackage.objects.create(
            name=name,
            includes_disassembly=includes_disassembly,
...
def animal_factory(db, slaughter_order_factory):
    ...
    def create_animal(
        slaughter_order=None,
        animal_type='cattle',
        identification_tag=None,
        status='received',
        **kwargs
    ):
...
def sample_order_with_animals(db):
    ...
    for i, animal_type in enumerate(['cattle', 'sheep', 'goat']):
        animal = AnimalFactory.create(
            ...
            animal_type=animal_type,
            identification_tag=f"TEST-{animal_type.upper()}-{i:03d}"
        )
...
def db_setup(db):
    ...
    full_service = ServicePackageFactory.create(
        name='Full Service',
        includes_disassembly=True,
        includes_delivery=True
    )
