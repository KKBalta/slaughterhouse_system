    ...
    def handle(self, *args, **options):
        ...
        for report_data in all_reports:
            ...
            if created:
                created_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f'Created report: {report.name}')
                )
            else:
                self.stdout.write(
                    self.style.WARNING(f'Report already exists: {report.name}')
                )

        self.stdout.write(
            self.style.SUCCESS(f'Successfully created {created_count} new reports')
        )
