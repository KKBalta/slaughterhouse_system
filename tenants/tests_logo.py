...
class TenantLogoUploadToTest(SimpleTestCase):
    def test_path_uses_schema_and_slug_and_logo_suffix(self):
        ...
        self.assertEqual(path, "tenant_logos/isim_et/isim-et-as_logo.png")
    ...
    def test_unknown_extension_defaults_to_png(self):
        ...
        self.assertEqual(path, "tenant_logos/acme/acme_logo.png")

    def test_empty_company_uses_schema_in_filename(self):
        inst = SimpleNamespace(schema_name="pomet", company_name="", name="")
        path = tenant_logo_upload_to(inst, "logo.jpg")
        self.assertEqual(path, "tenant_logos/pomet/pomet_logo.jpg")


class TenantCompanyProfileLogoFormTest(TestCase):
    def test_clean_logo_rejects_non_image_extension(self):
    ...
    def test_clean_logo_accepts_png(self):
        if Image is None:
            self.skipTest("Pillow is required for ImageField validation")
        buf = BytesIO()
