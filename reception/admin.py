...
class SlaughterOrderAdmin(admin.ModelAdmin):
    ...
    def convert_to_registered_client(self, request, queryset):
        for order in queryset:
            if order.client:
                self.message_user(
                    request, f"Order {order.id} is already linked to a registered client.", level=messages.WARNING
                )
                continue

            if not order.client_name or not order.client_phone:
                self.message_user(
                    request, f"Order {order.id} is missing a client name or phone number.", level=messages.ERROR
                )
                continue
            ...
            SlaughterOrder.objects.filter(client_name=order.client_name, client_phone=order.client_phone).update(
