...
class ReceptionServiceTest(TestCase):
    ...
    def test_update_slaughter_order_service(self):
        ...
        updated_order = update_slaughter_order(
            order=order, destination="New Market", service_package=self.service_package_simple
        )
    ...
    def test_update_order_status_from_animals_service(self):
        ...
        # Test COMPLETED status - process all animals through FSM transitions
        for animal in order.animals.all():
            # Reload from database to get fresh state (FSM doesn't support refresh_from_db)
            animal = Animal.objects.get(pk=animal.pk)

            # Transition through proper FSM sequence
    ...
    def test_create_batch_animals_with_received_date(self):
        ...
        created_animals = create_batch_animals(
            order=order, animal_type="cattle", quantity=3, received_date=custom_date, skip_photos=True
        )
    ...
    def test_generate_order_number_function(self):
        ...
        order1 = create_slaughter_order(
            ...
            client_phone="5551234567",  # Required field for walk-in clients
        )
        ...
        self.assertEqual(num_2, num_1 + 1, "Second order number should be one more than first")
